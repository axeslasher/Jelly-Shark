...
struct ServerOutageMonitor {
    ...
    /// how a test tells "the report landed" from "the verdict is in".
    private(set) var sampleCount = 0

    /// The verdict after the latest sample; nil while healthy.
    private(set) var outage: ServerOutage?

    /// The previous sample's playhead, so this one can be compared against
    /// it. Nil before the first sample and after `reset()`.
    ...
    ///     evidence either way
    ///   - transportStatus: a paused transport exempts the frozen playhead
    ...
    mutating func record(
        ...
        case let .failed(error):
            ...
            let stalled = consecutiveFailures >= stallFailureThreshold
                && frozen
