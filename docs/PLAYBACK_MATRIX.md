...
1. ✅ **Jellyfin echoes the previous selection back as the next session's default** (one session behind, observed across the eight-rebuild sweep: `default=` trailed `selection=` by exactly one). Any non-default choice therefore becomes `selection == default` on the item's next play. Since #259 resolves both indices the same way, the echo is cosmetic rather than a correctness hazard; the `selection=`/`default=` pair is still logged so the echo itself stays visible.
2. ✅ **The default branch's `FlagDefault` carry lies under that echo**: `selection == default` takes the frozen pre-#252 branch, which carries the file's `FlagDefault` track rather than the mapped default stream. Observed live three times (`default audio: carrying file track 2 but stream 2 maps to track 3`) — the committed English stream showed in the picker while the file-default French track played. Every explicit switch took the #252 mapped path and was correct.
   - ✅ **Fixed in #259, device-verified 2026-08-19** (SDR-panel Apple TV, same dual-language dual-E-AC-3 source, three plays): a fresh play defaults to the file-default-language track; a mid-play switch to the other language followed by a back-out and replay — the exact echo shape that produced the lie — plays the switched track under a truthful checkmark; and a default changed from another client (web) is honored on the next in-app play. The default index now resolves through the same stream-index mapping as a selection, so the file's `FlagDefault` no longer decides anything, and an inconclusive mapping on the default index server-transcodes that index instead of carrying the remuxer's pick (a silent wrong-track carry the instrumentation could not see). The `default audio: carrying file track N but stream M maps to track K` warning is deleted along with the field behind it. No shape changes rung: nothing that stayed on rung 1 descends, and nothing that descended now stays. The only outcome deltas are two wrong-track carries becoming honest server transcodes on the same rung — an inconclusive mapping on the default index (above), and a default index naming a sidecar while the file holds a carriable track, which used to carry an arbitrary embedded track under the sidecar's checkmark. Both are pinned by tests, as are the four shapes that name no mappable index.
